...
public class SmsMessageConsumer {
    ...
    private final SmsMessageSenderResolver smsMessageSenderResolver;

    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = MessageMqConstants.SMS_QUEUE, durable = "true"),
                    exchange = @Exchange(value = SystemConstants.SYS_MESSAGE_RABBITMQ_EXCHANGE),
                    key = MessageMqConstants.SMS_QUEUE
            )
    )
    public void onMessage(Long id,
                          Channel channel,
                          @Header(AmqpHeaders.DELIVERY_TAG) long tag) throws IOException {
        smsMessageSenderResolver.sendMessage(id);
