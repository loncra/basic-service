...
@Plugin(
        ...
        id = "enterprise_role",
        parent = "organization",
        authority = "perms[auth_server_enterprise_role:find]",
...
public class EnterpriseRoleController {
     ...
     */
    @PostMapping("find")
    @PreAuthorize("hasAuthority('perms[auth_server_enterprise_role:find]')")
    ...
    @PutMapping
    @OperationDataTrace
    @PreAuthorize("hasAuthority('perms[auth_server_enterprise_role:save]')")
    @Plugin(name = "添加或保存信息")
    ...
    @OperationDataTrace
    @Plugin(name = "删除信息")
    @PreAuthorize("hasAuthority('perms[auth_server_enterprise_role:delete]')")
